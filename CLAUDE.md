...
- **`MediaItem.title` and `MediaItem.qualitySources` are optional, scanner-provided metadata**: a title names the saved file instead of the URL's last segment, and `qualitySources` lets one item stand for a host's separate per-quality manifests (the popup's `hls-sources` picker context builds the quality list from it without fetching a master). `MediaStore.add` keeps an existing item's `title`/`filename` when the same URL is re-added without one, and drops generic sightings of URLs already listed in an item's `qualitySources` — the network watcher re-reports manifests as `master.m3u8` when the player fetches them. Nothing in `src/content/scan.ts` sets either field yet.
